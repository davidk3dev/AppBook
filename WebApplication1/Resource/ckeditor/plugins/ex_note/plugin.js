...
CKEDITOR.plugins.add( 'ex_note', {
	...
	init: function( editor ) {
		...
		CKEDITOR.dialog.add( 'dlgNote', this.path + 'dialogs/dlgNote.js' );

		// Register the simplebox widget.
		editor.widgets.add( 'ex_note', {
            ...
            allowedContent:'span(ckwg,wgnote,ckwg-display-text,ckwg-content,ckwg-info,ckwg-close-button,icon)',

			// Minimum HTML which is required by this widget to work.
			requiredContent: 'span(wgnote)',

			// Define the template of a new Simple Box widget.
			// The template will be used when creating new instances of the Simple Box widget.
			template:
				'<span class="ckwg wgnote">' +
					'<span class="ckwg-display-text"></span>' +
                	'<span class="ckwg-content"></span>' +
					'<span class="ckwg-info"><span class="icon">i</span></span>' +
			...
			upcast: function( element ) {
				...
				return element.name == 'span' && element.hasClass( 'wgnote' );
			},
			...
			init: function() {
                ...
                if(elementContent && elementContent.getHtml()){
                    this.data.noidungchuthich = elementContent.getHtml();
                }
                if(!selectedText){
				...
				//Init data
				this.data.tucanchuthich = selectedText;
			...
			data: function() {
                var elementContent = this.element.findOne( '.ckwg-content' );
                if(this.data.noidungchuthich && elementContent){
                    elementContent.setText(this.data.noidungchuthich);
				}
			}
		} );
        editor.ui.addButton( 'ex_note', {
            label: 'Note',
            command: 'ex_note',
            toolbar: 'newgroup,1'
        })
